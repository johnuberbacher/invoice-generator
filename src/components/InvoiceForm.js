...
class InvoiceForm extends React.Component {
  ...
  handleAddEvent(evt) {
    ...
    // Clone the current items array, add the new item, and set state
    const updatedItems = [...this.state.items, newItem];
    this.setState({ items: updatedItems }, () => {
      // After updating the items, recalculate the subtotal
      this.handleCalculateTotal();
    });
  }
  ...
  handleCalculateTotal() {
    ...
    // Check for valid taxRate and discountRate
    const taxRate = parseFloat(this.state.taxRate);
    const discountRate = parseFloat(this.state.discountRate);

    if (!isNaN(taxRate) && !isNaN(discountRate)) {
      ...
      // Handle invalid taxRate or discountRate (e.g., display an error message)
      console.error("Invalid taxRate or discountRate");
    }
  }

  handleTaxRateChange(event) {
    this.setState(
      { taxRate: event.target.value, taxAmount: event.target.value },
      () => {
        this.handleCalculateTotal();
      }
    );
  ...
  handleDiscountRateChange(event) {
    this.setState(
      { discountRate: event.target.value, discountAmount: event.target.value },
      () => {
        this.handleCalculateTotal();
      }
    );
